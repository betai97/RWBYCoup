...
public class GameScene2Controller implements Initializable {
    ...
    @FXML
    private ImageView player1card1, player1card2, playercard1, playercard2, rwbyidentity, light;
    @FXML
    Label player1coins, playercoins, decknum, player, player1;
    
   public void sendmessage(KeyEvent event) {
       if(event.getCode() == KeyCode.ENTER) {
           Client.writeToOS('^' + textfield.getText());    // append with '^' to indicate to the server the message is chat text, and stop chat text from being treated as command
           textfield.setText("");
       }
   }
   ...
   public void loseCard2() throws FileNotFoundException {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ImageView[] cards = {playercard1, playercard2, player1card1, player1card2};
        Label[] coins = {playercoins, player1coins};
        Label[] names = {player, player1};
